...
(async()=>{
 ...
 async function download(prefix){
  ...
  const item=await pending;assert.equal(await item.failure(),null);const data=fs.readFileSync(await item.path());
 ...
 await page.locator('#pt-src').fill(original);assert.equal(await page.locator('#pt-wav-filename').textContent(),'pocket_tunnel_arranged.wav');
 await page.locator('#pt-tab-jazz').click();assert.equal(await page.locator('#pt-wav-filename').textContent(),'pocket_tunnel_jazz.wav');
 await page.locator('#pt-tab-original').click();assert.equal(await page.locator('#pt-wav-filename').textContent(),'pocket_tunnel_arranged.wav');
