...
(async()=>{
 ...
 for(const published of [false])for(const folder of ['03_Windward-Crossing']) {
  ...
  await page.evaluate(()=>{window.measure=[];const P=MMSXX.sound.ChipTuneSound.prototype,original=P._scheduleTrack;P._scheduleTrack=function(...args){const t=performance.now();const r=original.apply(this,args);measure.push({ms:performance.now()-t,nodes:args[3].length,from:args[4],to:args[5],ch:args[0].ch,notes:args[0].events.filter(e=>e.t>=args[4]&&e.t<args[5]).map(e=>e.t)});return r;};});
