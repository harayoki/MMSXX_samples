...
`player-engine.js` を使用する。共通プレイヤーはサウンドエンジンと再生UIをバンドルしたもの。

## PLAYER更新・引継ぎルール

- `player-engine.js` は全サンプル共通。変更は全曲の再生に影響するため、1曲の音作り・不具合対応を目的とした独自変更を入れない。取得元から更新する場合も、全サンプルへの影響を確認する。

- PLAYER／サウンドエンジンは別のリポジトリから取得する。更新時は取得元と取り込み済みコピーを比較する。
- Sample側でエンジン本体を独自改造しない。減衰計算、音量計算、MML解析、再生処理などを書き換えない。
- 曲固有の音色・エンベロープ・音量設定は、公開APIを使って各曲の `player.js` またはMMLに記述する。
- エンジン本体の修正が必要な場合は、問題と根拠を報告し、明示指示を待つ。
- 非公開リポジトリのURLをSample側に記載しない。MML内のエンジン名表記は残してよい。
