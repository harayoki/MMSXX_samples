...
標準値はPocket Tunnelが1、Volcano Paradeが2。開始側は `[`、終了側は `]$LOOP_END` とする。更新したエンジンはマクロ展開時に空白を足さないため、数字だけのマクロをそのまま扱える。ページ側でMMLを別の構文へ書き換える処理は行わない。

マクロは文字として連結される。例えば `$len = { 4 }` を `a$len` と呼ぶと `a4` になる。意図しない連結を避ける箇所では呼び出し側に空白を入れる。なお、この版では数字もマクロ名に含まれるため、`$a4` は `a4` という名前のマクロとして扱われる。
...
ボタンは「WAVとしてダウンロード」、補足は「MMLは編集できます。 WAV出力にも反映されます。」とする。ページにファイル名や成功メッセージは出さない。ダウンロード名の `_arranged` 規則は維持する。

生成中は中央のモーダルダイアログで背景を暗くし、他の操作とスクロールを防ぐ。進捗は実際の OfflineAudioContext の処理時刻（0〜95%）とPCM変換済みサンプル数（95〜100%）から表示する。MML準備中は0%で表示する。

キャンセルボタンまたはEscapeで中止できる。音声生成はOfflineAudioContextをsuspendし、PCM変換も中断してダウンロードしない。通常の楽曲再生は継続する。完了・中止時はダイアログを閉じ、ダウンロードボタンにフォーカスを戻す。エラー時のみページにメッセージを残す。

風渡りの境界も同じWAV UIを使用し、保存名は `windward_crossing.wav`。既存組曲の順序・区間の反復回数は維持する。
